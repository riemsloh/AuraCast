...
struct ContentView: View {
    ...
    @ObservedObject var forecastModel: ForecastViewModel // 5 Tage Vorhersage Model
    
    var body: some View {
        // Korrekte Syntax für NavigationSplitView mit drei Spalten
        NavigationSplitView {
            // MARK: - Sidebar (erste Spalte)
            List {
                Section("Garten") {
                    NavigationLink {
                    ...
                    NavigationLink {
                        // Da hier keine Destination definiert ist, wird nichts passieren,
                        // oder es würde eine leere Ansicht angezeigt, wenn dieser Link aktiv wird.
                        // HistoryOneDay(wunderground: $wunderground, historyDate: $historyDate) // Auskommentiert wie in deinem Snippet
                        Text("Tages Übersicht Detail") // Platzhalter für jetzt
                    } label: {
                        Label("Tages Übersicht", systemImage: "calendar") // Passenderes Symbol
                    }
                }
            }
            .navigationTitle("Das Wetter heute") // Titel für die Sidebar
        } detail: {
            // MARK: - Detail (dritte Spalte)
            // Dies ist die Standardansicht, die angezeigt wird, wenn nichts anderes ausgewählt ist.
           // UnaviableView()
        }
            ...
            VStack(spacing: 20) {
                ...
                Button("Laden") {
                    Task {
                        ...
                        await forecastModel.fetchForecastData()
                    }
                }
